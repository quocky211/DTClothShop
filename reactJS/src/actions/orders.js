...
export const vnpayPayment = async (total, order_id) => {
  const res = await axios.post('https://thawing-hollows-39647.herokuapp.com/order/vnpay_url', { total,  order_id });
  return res.data;
};

export const vnPayReturn = async () => {
  const res = await axios.get(`https://thawing-hollows-39647.herokuapp.com/order/vnpay_return${window.location.search}`);
  return res.data;
};
